...
try{
 ...
 $q=$pdo->prepare("SELECT DATE_FORMAT(disconnected_at,'%Y-%m-%d %H:00:00') hour,COUNT(*) total FROM pppoe_disconnect_history WHERE disconnected_at>=DATE_SUB(NOW(),INTERVAL 24 HOUR)".$filter." GROUP BY DATE_FORMAT(disconnected_at,'%Y-%m-%d %H:00:00') ORDER BY hour");$q->execute($p);$raw=$q->fetchAll(PDO::FETCH_ASSOC);$counts=[];foreach($raw as $row)$counts[(string)$row['hour']]=(int)$row['total'];
 $hours=[];$base=new DateTime('now',new DateTimeZone('Asia/Jakarta'));$base->setTime((int)$base->format('H'),0,0);for($i=23;$i>=0;$i--){$t=clone $base;$t->modify('-'.$i.' hours');$key=$t->format('Y-m-d H:00:00');$hours[]=['hour'=>$t->format('H:00'),'total'=>$counts[$key]??0];}
 $q=$pdo->prepare("SELECT username,COUNT(*) disconnects,ROUND(AVG(NULLIF(uptime_seconds,0)),0) avg_uptime_seconds,MIN(disconnected_at) first_disconnect,MAX(disconnected_at) last_disconnect FROM pppoe_disconnect_history WHERE disconnected_at>=DATE_SUB(NOW(),INTERVAL 30 DAY)".$filter." GROUP BY username HAVING COUNT(*)>=2 ORDER BY disconnects DESC,last_disconnect DESC,username LIMIT 10");$q->execute($p);$topUnstable=$q->fetchAll(PDO::FETCH_ASSOC);
 $avgUptime=null;$q=$pdo->prepare("SELECT ROUND(AVG(NULLIF(uptime_seconds,0)),0) FROM pppoe_disconnect_history WHERE disconnected_at>=DATE_SUB(NOW(),INTERVAL 30 DAY)".$filter);$q->execute($p);$avgUptime=$q->fetchColumn();
 echo json_encode(['success'=>true,'summary'=>['today'=>$today,'week'=>$week,'month'=>$month,'avg_uptime_seconds'=>$avgUptime!==false&&$avgUptime!==null?(int)$avgUptime:null],'top_users'=>$users,'top_routers'=>$routers,'hours'=>$hours,'top_unstable'=>$topUnstable],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
