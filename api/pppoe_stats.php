...
try {
    ...
    foreach ($secretRows as $r) {
        ...
        $secretProfiles[(string)($r['name'] ?? '')] = (string)($r['profile'] ?? 'default');
    }
    ...
    foreach ($activeRows as $r) {
        ...
        $name = (string)($r['name'] ?? '-');
        $active[] = [
            'id'=>$r['.id']??'', 'name'=>$name, 'address'=>$r['address']??'-',
            'caller_id'=>$r['caller-id']??'-', 'uptime'=>$r['uptime']??'-', 'service'=>$r['service']??'-',
            'profile'=>$secretProfiles[$name] ?? 'default',
            'bytes_in'=>$rx, 'bytes_out'=>$tx,
            'packets_in'=>bytes_num($r['packets-in']??0),
