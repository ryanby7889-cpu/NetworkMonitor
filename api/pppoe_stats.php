...
try{
    ...
    foreach($activeRows as $r){if(!is_array($r))continue;$name=(string)($r['name']??'-');[$txPair,$rxPair]=pairv($r['bytes']??'');$rx=numv(firstv($r,['bytes-in','rx-byte','rx_bytes'],0));$tx=numv(firstv($r,['bytes-out','tx-byte','tx_bytes'],0));if($rx<=0&&$rxPair>0)$rx=$rxPair;if($tx<=0&&$txPair>0)$tx=$txPair;
