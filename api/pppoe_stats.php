...
function numv($v){
    ...
    if(preg_match('/^([+-]?(?:\d+(?:\.\d*)?|\.\d+))([kmgtpe]?i?b|[kmgtpe])$/i',$s,$m)){
        $n=(float)$m[1];$u=strtolower($m[2]);$binary=strpos($u,'i')!==false;$unit=preg_replace('/i?b$/','',$u);$base=$binary?1024:1000;$powers=['k'=>1,'m'=>2,'g'=>3,'t'=>4,'p'=>5,'e'=>6];return $n*($powers[$unit]??0 ? pow($base,$powers[$unit]) : 1);
    }
    return 0.0;
}
function pairv($v){
...
function norm_if($s){$s=strtolower(trim((string)$s));return trim($s,'<>');}
function record_pppoe_history($active){
    try{
        ...
        foreach($active as $x){$q->execute([$x['name'],$x['session_id']??null,$x['address']??null,$x['interface']??null,$x['profile']??null,(int)max(0,$x['bytes_in']??0),(int)max(0,$x['bytes_out']??0),(int)max(0,$x['packets_in']??0),(int)max(0,$x['packets_out']??0)]);}
    }catch(Throwable $e){/* History must never break live PPPoE monitoring. */}
}
try{
    ...
    $ifRows=(array)$api->comm('/interface/print',['.proplist'=>'.id,name,type,rx-byte,tx-byte,rx-packet,tx-packet']);$ifMap=[];
    foreach($ifRows as $ir){if(!is_array($ir))continue;$iname=(string)($ir['name']??'');$key=norm_if($iname);if($key!=='')$ifMap[$key]=$ir;if(strpos($key,'pppoe-')===0)$ifMap[substr($key,6)]=$ir;}
    ...
    foreach($activeRows as $r){
        if(!is_array($r))continue;$name=(string)($r['name']??'-');[$txPair,$rxPair]=pairv($r['bytes']??'');
        ...
        if($ir){$pin=numv(firstv($ir,['rx-packet'],$pin));$pout=numv(firstv($ir,['tx-packet'],$pout));}if($pin<=0&&$pktRx>0)$pin=$pktRx;if($pout<=0&&$pktTx>0)$pout=$pktTx;
    ...
    usort($active,fn($a,$b)=>$b['total_bytes']<=>$a['total_bytes']);record_pppoe_history($active);$api->disconnect();
