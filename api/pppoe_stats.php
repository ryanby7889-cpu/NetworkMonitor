...
try{
    ...
    foreach($secretRows as $r){
        if(is_array($r)) $secretMap[(string)($r['name']??'')]=['profile'=>$r['profile']??'default','service'=>$r['service']??'pppoe'];
    }
    ...
    foreach($activeRows as $r){
        ...
        $active[]=[
            'id'=>$r['.id']??'','name'=>$name,'address'=>$r['address']??'-','caller_id'=>$r['caller-id']??'-','uptime'=>$r['uptime']??'-',
            'service'=>$r['service']??($secretMap[$name]['service']??'pppoe'),'profile'=>$secretMap[$name]['profile']??'default',
            'bytes_in'=>$rx,'bytes_out'=>$tx,'total_bytes'=>$rx+$tx,'packets_in'=>$pin,'packets_out'=>$pout,'session_id'=>$r['session-id']??'-'
        ];
    }
    ...
    foreach($secretRows as $r){
        if(!is_array($r)) continue;
        $d=(($r['disabled']??'false')==='true'); if($d)$disabled++;else$enabled++;
    ...
    $api->disconnect();
    pppoe_json(true,'',[
        'active_count'=>count($active),'account_count'=>count($secretRows),'profile_count'=>count($profileRows),
        'enabled_accounts'=>$enabled,'disabled_accounts'=>$disabled,'total_rx_bytes'=>$totalRx,'total_tx_bytes'=>$totalTx,'total_traffic_bytes'=>$totalRx+$totalTx,
        'active'=>$active,'top_users'=>array_slice($active,0,5),'services'=>$services,'profile_usage'=>$profileUsage,'updated_at'=>date('Y-m-d H:i:s')
    ]);
