...
try {
  ...
  $pdo=getDb();
  $pdo->exec("CREATE TABLE IF NOT EXISTS pppoe_traffic_history (id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, router_id INT NOT NULL DEFAULT 1, username VARCHAR(128) NOT NULL, session_id VARCHAR(128) NULL, interface_name VARCHAR(128) NULL, profile VARCHAR(128) NULL, bytes_in BIGINT UNSIGNED NOT NULL DEFAULT 0, bytes_out BIGINT UNSIGNED NOT NULL DEFAULT 0, packets_in BIGINT UNSIGNED NOT NULL DEFAULT 0, packets_out BIGINT UNSIGNED NOT NULL DEFAULT 0, recorded_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(id), KEY idx_user_time(username,recorded_at), KEY idx_time(recorded_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
  $stmt=$pdo->query("SELECT username,MAX(profile) profile,MIN(recorded_at) first_at,MAX(recorded_at) last_at,MAX(bytes_in)-MIN(bytes_in) upload_bytes,MAX(bytes_out)-MIN(bytes_out) download_bytes,COUNT(*) records FROM pppoe_traffic_history WHERE recorded_at>=DATE_SUB(NOW(), INTERVAL {$allowed[$range]}) GROUP BY username ORDER BY (download_bytes+upload_bytes) DESC");
  $rows=[];foreach($stmt as $r){$rows[]=['username'=>$r['username'],'profile'=>$r['profile']??'','download_bytes'=>(float)$r['download_bytes'],'upload_bytes'=>(float)$r['upload_bytes'],'total_bytes'=>(float)$r['download_bytes']+(float)$r['upload_bytes'],'records'=>(int)$r['records'],'first_at'=>$r['first_at'],'last_at'=>$r['last_at']];}
