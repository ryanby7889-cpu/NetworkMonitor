...
try {
  ...
  $rows=[];
  foreach($stmt as $r){
    $rows[]=['username'=>$r['username'],'profile'=>$r['profile']??'','download_bytes'=>max(0,(float)$r['download_bytes']),'upload_bytes'=>max(0,(float)$r['upload_bytes']),'total_bytes'=>max(0,(float)$r['download_bytes'])+max(0,(float)$r['upload_bytes']),'records'=>(int)$r['records'],'first_at'=>$r['first_at'],'last_at'=>$r['last_at']];
  }
