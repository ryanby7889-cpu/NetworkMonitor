...
try {
 ...
 $rows=$pdo->query("SELECT alarm_type,severity,status,interface_name,value,threshold,created_at,resolved_at FROM alarms WHERE created_at>=DATE_SUB(NOW(),INTERVAL 30 DAY) AND (LOWER(alarm_type) LIKE 'pppoe%' OR LOWER(interface_name) LIKE '%pppoe%') ORDER BY created_at DESC")->fetchAll(PDO::FETCH_ASSOC);
 $users=[];
