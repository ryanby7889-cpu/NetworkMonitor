(() => {
  ...
  function aggregate(data) {
    ...
    return [...buckets.values()].sort((a,b)=>a.created_at-b.created_at).map(b => ({created_at:b.created_at.toLocaleString('sv-SE').replace('T',' '),interface_name:b.interface_name,download_mbps:b.count?b.download/b.count:0,upload_mbps:b.count?b.upload/b.count:0,rx_packet:b.rx,tx_packet:b.tx,cpu:b.count?b.cpu/b.count:0,memory:b.count?b.memory/b.count:0,disk:b.count?b.disk/b.count:0}));
  }
  ...
  function render(data) {
    ...
    const down = chartData.map(x => Number(x.download_mbps)||0), up = chartData.map(x => Number(x.upload_mbps)||0);
    ...
    const avgUp = filtered.length ? filtered.reduce((s,x)=>s+(Number(x.upload_mbps)||0),0)/filtered.length : 0;
    const peak = filtered.reduce((p,x)=>{const v=Number(x.download_mbps)||0;if(!p||v>p.v)return {v,time:x.created_at};return p},null);
    $('statRecords').textContent=filtered.length.toLocaleString();
    $('statMaxDownload').textContent=fmt(maxDown)+' Mbps'; $('statMaxUpload').textContent=fmt(maxUp)+' Mbps';
    $('statAvgDownload').textContent=fmt(avgDown)+' Mbps'; $('statAvgUpload').textContent=fmt(avgUp)+' Mbps';
    $('statPeakTime').textContent=peak?.time ? new Date(String(peak.time).replace(' ','T')).toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'}) : '-';
    $('statPeakTime').title=peak?.time ? `Peak download ${fmt(peak.v)} Mbps pada ${peak.time}` : '';
  ...
  async function load() {
    try { const r=await fetch(`../api/traffic_history.php?range=${encodeURIComponent(range)}&router_id=${routerId}&limit=500&nocache=${Date.now()}`,{cache:'no-store'}); const d=await r.json(); if(!d.success) throw Error(d.message||'Gagal memuat histori'); $('chartPeriod').textContent=`${d.from} s/d ${d.to}`; render(d.data||[]); const ex=$('exportTraffic'); if(ex) ex.href=`export.php?range=${range}&router_id=${routerId}`; }
  ...
  async function init(){
    ...
    const intSel=$('historyInterface'); if(intSel) intSel.addEventListener('change',()=>{interfaceName=intSel.value;render(window.__trafficHistoryData||[]);});
