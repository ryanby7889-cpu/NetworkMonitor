(() => {
  ...
  function render(d){
    ...
    $('monthly').innerHTML=(d.trend||[]).map(x=>{const rate=Number(x.billed)>0?(Number(x.paid)/Number(x.billed)*100):0;return `<tr class="${x.month===d.month?'current':''}"><td>${monthLabel(x.month)}</td><td>${num(x.invoice_count)}</td><td>${money(x.billed)}</td><td>${money(x.paid)}</td><td>${money(x.unpaid)}</td><td>${rate.toFixed(1)}%</td></tr>`;}).join('');
  }
  async function load(){
